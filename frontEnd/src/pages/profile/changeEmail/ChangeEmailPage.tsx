  ...
  return (
    <>
      ...
      <FormProvider {...requestForm}>
        <form
          onSubmit={requestForm.handleSubmit(onRequestSubmit, commonProps.onFormError)}
          style={{ display: step === 'verify' ? 'block' : 'none' }}
          autoComplete="off"
          noValidate
        >
          <StandardRequestForm isSubmitting={isRequesting} />
        </form>
      </FormProvider>
      <VerifyFlow
        step={step}
        newEmail={requestForm.watch('email')}
...
function KakaoChangeEmail() {
  ...
  const requestForm = useForm<KakaoRequestFormValues>({
    resolver: zodResolver(kakaoRequestSchema),
    mode: 'onBlur',
  });
  ...
  const onRequestSubmit: SubmitHandler<KakaoRequestFormValues> = async (data) => {
    await requestVerification({ newEmail: data.email });
  };
    ...
    <>
      ...
      <FormProvider {...requestForm}>
        <form
          ...
          noValidate
        >
          <KakaoRequestForm isSubmitting={isRequesting} />
        </form>
      </FormProvider>
      <VerifyFlow
        step={step}
...
function useEmailChangeFlow() {
  ...
  const onFormError = (errors: any) => {
    ...
    if (firstErrorKey) {
      appToast.error({ message: errors[firstErrorKey].message as string });
    }
  };
...
function VerifyFlow({ step, newEmail, onGoBack, ...props }: any) {
  ...
  return (
    <FormProvider {...props.verifyForm}>
      <form
        onSubmit={props.verifyForm.handleSubmit(onVerifySubmit, props.onFormError)}
        style={{ display: step === 'code' ? 'block' : 'none' }}
      >
        <VerifyForm
          isSubmitting={props.isExecuting}
          isResending={props.isResending}
          newEmail={newEmail}
          timeRemaining={props.verificationTime}
          isTimerRunning={props.isVerificationTimerRunning}
          resendCooldown={props.resendCooldown}
          isResendCooldown={props.isResendCooldown}
          resendCount={props.resendCount}
          onResend={() => props.handleResend(newEmail)}
          onGoBack={onGoBack}
          formatTime={props.formatTime}
          verificationErrorCount={props.verificationErrorCount}
          isMaxErrorReached={props.isMaxErrorReached}
        />
      </form>
    </FormProvider>
  );
...
const KakaoRequestForm = ({ isSubmitting }: { isSubmitting: boolean }) => {
  ...
  return (
    <Card className="p-6 space-y-6">
      <div>
        <h2 className="text-xl font-bold mb-2">새 이메일 입력</h2>
