...
export async function handleServerError(appError: AppError): Promise<boolean> {
  ...
  if (!appError.code) {
    ...
    appToast.error({
      message: appError.AppErrorPolicyMap.title || '알 수 없는 오류가 발생했습니다.',
    });
    return false;
  }

  const policy: AppErrorPolicyMap = appError.AppErrorPolicyMap;
  ...
  if (policy.displayType === 'toast') {
    ...
    appToast[policy.type]({ message: appError.AppErrorPolicyMap.title });
    return false;
  }
  ...
  if (policy.displayType === 'confirmModal') {
  ...
  // 🟢 Alert 모달 (단순 고지)
  else if (policy.displayType === 'alertModal') {
    const modalStore = useModalStore.getState();
    const modalProps = {
      title: policy.title,
      description: policy.description,
